// app/api/customers/pending/[id]/route.js
...
export async function GET(req, { params }) {
  ...
  logActivity({
    ...
    action: "pending_customer_view",
    category: "CRUD",
    description: `Viewed pending customer "${customer.name}" — Roll: ${customer.rollNo || "N/A"}`,
    meta: {
...
export async function PATCH(req, { params }) {
  ...
  const before = await db.collection("pendingCustomers").findOne({ _id });
  ...
  await addCommit({
    ...
    action: "EDIT_PROFILE",
    meta: { source: "PENDING", fields: changedFields },
  });
  ...
  logActivity({
    userId: session.userId,
    username: session.username,
    action: "pending_customer_edit",
    category: "CRUD",
    description: `Edited pending "${before?.name || "unknown"}" — ${changedFields.length} field(s): ${changedFields.join(", ")}`,
    meta: {
      customerId: String(_id),
      name: before?.name,
