// app/api/customers/today/[id]/route.js
...
export async function GET(req, { params }) {
  ...
  logActivity({
    ...
    category: "CRUD",
    description: `Viewed customer "${customer.name}" — Roll: ${customer.rollNo || "N/A"}`,
    meta: {
...
export async function PATCH(req, { params }) {
  ...
  const before = await db.collection("todayCustomers").findOne({ _id });
  ...
  logActivity({
    ...
    category: "CRUD",
    description: `Edited customer "${before?.name || "unknown"}" — ${changedFields.length} field(s): ${changedFields.join(", ")}`,
    meta: {
      customerId: String(_id),
      name: before?.name,
