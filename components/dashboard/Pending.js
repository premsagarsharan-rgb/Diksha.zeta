...
function StatusBadge({ status }) {
  ...
  return (
    <div className={`mt-3 inline-flex px-2 py-1 rounded-full text-[11px] border ${cls}`}>
      Status: {s}
    </div>
  );
}
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
            ...
            <div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {recent24.map((c) => (
                  ...
                  >
                    <div className="text-xs text-white/70">Pending Profile</div>
          ...
          {older.length > 0 ? (
            <div>
              <div className="text-xs text-white/60 mb-2">Others — {older.length}</div>
